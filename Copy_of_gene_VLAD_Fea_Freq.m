...
function [] = gene_FV_Fea_Freq()
    ...
    for iter1 = 1:nFea
        ...
        nn = vl_kdtreequery(kdtree, centers, (real(feaSet.kernel_domFeaArr))');
        
        assignments = zeros( size(centers, 2), size(feaSet.kernel_domFeaArr, 1));
        assignments(sub2ind(size(assignments), nn, 1:length(nn))) = 1;
        
        fea = vl_vlad((real(feaSet.kernel_domFeaArr))',centers,assignments, 'NormalizeComponents' );  %, 'NormalizeComponents'
        %fea = fea / norm(fea);
