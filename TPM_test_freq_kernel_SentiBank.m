...
function TPM_FreqDom_test()
    ...
    dFea = 2048+4000;%sum(nCodebook * pyramid);
    
    load('dictionary/feaDatabase_freq_avg_kernel.mat');
    feaDatabase_Freq = feaDatabase_freq_avg_kernel;
    ...
    for ii = 1:nRounds
        ...
        for jj = 1:length(tr_idx)
            ...
            fea(1:2048) = fea(1:2048)*3000;
            %fea(2049:end) = fea(2049:end)/norm(fea(2049:end));
            %fea(2049:end) = fea(2049:end)/norm(fea(2049:end));
            fea = fea/norm(fea);
            tr_fea(jj, :) = fea;
            tr_label(jj) = label;
